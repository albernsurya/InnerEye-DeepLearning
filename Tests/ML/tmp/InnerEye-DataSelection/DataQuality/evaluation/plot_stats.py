...
import numpy as np
import pandas as pd
from DataQuality.selection.simulation_statistics import SimulationStatsDistribution
...
from DataQuality.utils.generic import create_folder, save_obj
...
def plot_relabeling_score(true_majority: np.ndarray,
    ...
    """
    create_folder(BENCHMARK2_DIR / "scores_histogram")
    create_folder(BENCHMARK2_DIR / "noise_detection_heatmaps")
    ...
    plt.savefig(BENCHMARK2_DIR / "scores_histogram" / selector_name)
    plt.close()
    ...
    plt.savefig(BENCHMARK2_DIR / "noise_detection_heatmaps" / selector_name)
    plt.close()
