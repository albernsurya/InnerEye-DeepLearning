...
from deep_learning.collect_embeddings import get_all_embeddings
from deep_learning.loss import CrossEntropyLoss
from deep_learning.model_trainer_base import Loss
from deep_learning.vanilla_trainer import VanillaTrainer
...
class ELRTrainer(VanillaTrainer):
    ...
    def compute_loss(self, is_train: bool, outputs: List[torch.Tensor], labels: torch.Tensor,
        ...
        logits = outputs[0]
        if is_train:
            per_sample_loss = self.loss_fn["TRAIN"](predictions=logits, targets=labels, indices=indices)
        else:
            per_sample_loss = self.loss_fn["VAL"](predictions=logits, targets=labels, reduction='none')
