...
from DataQuality.deep_learning.metrics.sample_metrics import SampleMetrics
from DataQuality.deep_learning.plots_tensorboard import (get_scatter_plot, plot_disagreement_per_sample,
                                                         plot_excluded_cases_coteaching)
from DataQuality.deep_learning.transforms import ToNumpy
from DataQuality.utils.plot import plot_latent_space_and_noise_cases
...
class JointMetrics():
    ...
    def log_embeddings(self, writer: SummaryWriter, labels: np.ndarray, embeddings: np.ndarray, epoch: int) -> None:
        ...
        """
        available_indices = ~np.isnan(labels)
        metadata = ["Mislabelled" if i in self.true_mislabelled_ids else "Clean" for i in range(self.num_samples)]
        labels = [self.label_names[int(x)] for x in labels[available_indices].tolist()]
        figure, ax = plt.subplots(figsize=(14, 12))
        plot_latent_space_and_noise_cases(embeddings=embeddings[available_indices, :],
                                          labels=np.array(labels),
                                          indicator_noisy_labels=np.asarray(metadata)[available_indices],
                                          selected_cases=self.case_drop_histogram[available_indices, epoch],
                                          ax=ax)
        writer.add_figure(tag="Embeddings", figure=figure, global_step=epoch)
        print("log embeddings done")
