...
from DataQuality.deep_learning.self_supervised.byol.byol_module import BYOLModule
from DataQuality.deep_learning.self_supervised.datamodules.utils import create_ssl_data_modules
from DataQuality.deep_learning.self_supervised.simclr_module import SimCLRModule
from DataQuality.deep_learning.self_supervised.ssl_classifier_module import (SSLOnlineEvaluatorModule,
                                                                             get_encoder_output_dim)
from DataQuality.deep_learning.utils import load_ssl_model_config
from DataQuality.utils.default_paths import EXPERIMENT_DIR
...
def cli_main(config: ConfigNode, debug: bool = False) -> None:
    ...
    if ssl_type == "simclr":
        model = SimCLRModule(num_samples=dm.num_samples,  # type: ignore
    ...
    # Create BYOL model
        ...
        model = BYOLModule(num_samples=dm.num_samples,  # type: ignore
    ...
    online_eval = SSLOnlineEvaluatorModule(class_weights=dm.class_weights,  # type: ignore
...
if __name__ == "__main__":
    ...
    # Launch the script
    cli_main(config)
    # Wait 10 minutes before stopping to give AML time to save checkpoint before leaving the job (checkpoint
    # corruption)
    time.sleep(600)
