...
import torch
from DataQuality.deep_learning.self_supervised.ssl_classifier_module import PretrainedClassifier, SSLClassifier, \
    WrapSSL
...
def create_ssl_image_classifier(num_classes: int, pl_checkpoint_path: str, freeze_encoder: bool = True) -> torch.nn.Module:
    ...
    if ssl_type == "byol":
        byol_module = WrapSSL(BYOLModule, num_classes).load_from_checkpoint(pl_checkpoint_path, strict=False)
        if freeze_encoder:
        ...
        simclr_module = WrapSSL(SimCLRModule, num_classes).load_from_checkpoint(pl_checkpoint_path, strict=False)
        if freeze_encoder:
