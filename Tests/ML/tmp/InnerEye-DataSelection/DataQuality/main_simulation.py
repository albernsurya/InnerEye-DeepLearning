...
import numpy as np
from DataQuality.evaluation.plot_stats import plot_stats
from DataQuality.selection.data_curation_utils import get_user_specified_selectors, \
    update_trainer_for_simulation
from DataQuality.selection.selectors.base import SampleSelector
from DataQuality.selection.selectors.label_based import (LabelBasedDecisionRule, LabelDistributionBasedSampler,
                                                         PosteriorBasedSelector)
from DataQuality.selection.selectors.bald import BaldSelector
from DataQuality.selection.selectors.random_selector import RandomSelector
from DataQuality.selection.simulation import DataCurationSimulator
from DataQuality.selection.simulation_statistics import SimulationStats, SimulationStatsDistribution
from DataQuality.utils.dataset_utils import load_dataset_and_initial_labels_for_simulation
from DataQuality.utils.default_paths import BENCHMARK2_DIR
from DataQuality.utils.generic import create_folder, get_data_selection_parser, get_logger, set_seed
from joblib import Parallel, delayed

EXP_OUTPUT_DIR = BENCHMARK2_DIR / datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
...
def main(args: argparse.Namespace) -> None:
    ...
    def _run_simulation_for_selector(name: str,
        ...
        if sample_selector.output_directory is not None:
            simulator.save_simulator_results(BENCHMARK2_DIR / sample_selector.output_directory / f"seed_{seed}")
