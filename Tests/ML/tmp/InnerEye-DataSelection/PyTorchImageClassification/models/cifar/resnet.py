...
class Network(nn.Module):
    def __init__(self, config):
        ...
        if model_config.apply_l2_norm:
            self.embedding_norm_fn = lambda x: torch.nn.functional.normalize(x, p=2, dim=1)
        else:
            self.embedding_norm_fn = None
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        if self.embedding_norm_fn is not None:
            x = self.embedding_norm_fn(x)
        x = self.dropout_layer(x) if self.use_dropout else x
        x = self.fc(x)
        return x
